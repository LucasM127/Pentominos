        ...
        for(uint j= 0; j<level.height; j++)
    {
        ...
        if(checkBit(data, i))//  m_data[C.j], C.i))
        {
            Coord C(i + x_offset, j + y_offset);
                ...
                grid.setCellTexture(C,getTextureUV(TexAtlasID::WINZONE_TEXTURE),getTextureSize());
        }
    }
...
void HelpState::position()
{
    ...
    for(uint i = 0; i < m_texts.size(); ++i)
    {
        ...
        if(i%2)
            m_texts[i].setPosition(5.f * cellSz + grid.getOffset().x, float(i + 1) * cellSz + grid.getOffset().y);
        else
            m_texts[i].setPosition(3.f * cellSz + grid.getOffset().x, float(i + 1) * cellSz + grid.getOffset().y); 
    }
}
